...
namespace ProjectApp
{
    public partial class MapPage : ContentPage
    {
        ...
        private Services.GeofencingService _geofencingService = new Services.GeofencingService();
...
<body>
    ...
    <script>
        ...
        {
            // Request location permission at runtime (required on Android)
            ...
            if (status != PermissionStatus.Granted)
            {
                ...
                if (status != PermissionStatus.Granted)
                {
                    ...
                    await DisplayAlert("Quyền vị trí", "Ứng dụng cần quyền truy cập vị trí để hiển thị vị trí của bạn trên bản đồ.", "OK");
                    return;
                }
            }
            ...
            while (true)
            {
                try
                {
                    ...
                    if (location != null && isPageLoaded)
                    {
                        ...
                        // Check Geofence and Auto Play TTS
                        await _geofencingService.CheckNearbyRestaurant(location.Latitude, location.Longitude);
                    }
                }
                catch (Exception ex)
                {
                ...
                await Task.Delay(3000); // Update Location (3s) 
            }
        }
    }
}