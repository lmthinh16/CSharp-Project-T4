    ...
    public class AnalyticsPage : ContentPage
    {
        ...
        private async Task LoadAsync()
        {
            ...
            try
            {
                ...
                // ── Recent activities ──────────────────────────────
                var recentTitle = _currentLang switch { "en" => "🕒  Recent Activities", "zh" => "🕒  最近活动", "ja" => "🕒  最近の活動", "ko" => "🕒  최근 활동", _ => "🕒  Hoạt động gần đây" };
                _content.Add(BuildSectionHeader(recentTitle));
                var allEvents = await App.Database.GetAllAnalyticsEventsAsync();
                var currentUser = UserSession.Instance.Username;
                var userEvents = allEvents.Where(e => e.Username == currentUser && e.PoiId > 0)
                ...
                if (userEvents.Count > 0)
                {
                    ...
                    _content.Add(BuildRecentActivities(userEvents, restaurants));
                }
                else
                {
                    var emptyMsg2 = _currentLang switch { "en" => "No recent activities.", "zh" => "暂无近期活动。", "ja" => "最近の活動はありません。", "ko" => "최근 활동이 없습니다.", _ => "Chưa có hoạt động gần đây." };
                    _content.Add(BuildEmptyState(emptyMsg2));
                }
        ...
        private Border BuildRecentActivities(List<AnalyticsEvent> events, List<Restaurant> restaurants)
        {
            var stack = new VerticalStackLayout { Spacing = 12, Padding = new Thickness(16) };
            for (int i = 0; i < events.Count; i++)
            {
                var ev = events[i];
                var r = restaurants.FirstOrDefault(x => x.Id == ev.PoiId);
                var dt = new DateTime(ev.TimestampTicks);
                var row = new Grid { ColumnDefinitions = { new ColumnDefinition { Width = 36 }, new ColumnDefinition { Width = GridLength.Star }, new ColumnDefinition { Width = GridLength.Auto } }, ColumnSpacing = 12 };
                
                string icon = ev.EventType.StartsWith("audio") ? "🎧" : "📍";
                string actionStr = ev.EventType.StartsWith("audio") 
                    ? (_currentLang switch { "en" => "Listened to", "zh" => "收听了", "ko" => "청취함", "ja" => "聴取しました", _ => "Nghe thuyết minh" })
                    : (_currentLang switch { "en" => "Visited", "zh" => "访问了", "ko" => "방문함", "ja" => "訪問しました", _ => "Đã đến thăm" });
                
                var iconBorder = new Border { BackgroundColor = Color.FromArgb("#F1F5F9"), StrokeThickness = 0, StrokeShape = new RoundRectangle { CornerRadius = 18 }, WidthRequest = 36, HeightRequest = 36 };
                iconBorder.Content = new Label { Text = icon, FontSize = 16, HorizontalOptions = LayoutOptions.Center, VerticalOptions = LayoutOptions.Center };
                row.Add(iconBorder, 0, 0);

                var infoStack = new VerticalStackLayout { Spacing = 2, VerticalOptions = LayoutOptions.Center };
                infoStack.Add(new Label { Text = actionStr, FontSize = 12, TextColor = TextSecond });
                infoStack.Add(new Label { Text = r?.DisplayName ?? "Địa điểm", FontSize = 14, FontAttributes = FontAttributes.Bold, TextColor = TextPrimary });
                row.Add(infoStack, 1, 0);

                var timeStr = dt.Date == DateTime.Today ? dt.ToString("HH:mm") : dt.ToString("dd/MM");
                row.Add(new Label { Text = timeStr, FontSize = 12, TextColor = TextSecond, VerticalOptions = LayoutOptions.Center }, 2, 0);

                stack.Add(row);
                if (i < events.Count - 1) stack.Add(new BoxView { HeightRequest = 1, BackgroundColor = Divider });
            }
            return new Border { BackgroundColor = BgCard, StrokeShape = new RoundRectangle { CornerRadius = 20 }, Padding = 0, Margin = new Thickness(16, 0), Content = stack };
        }
