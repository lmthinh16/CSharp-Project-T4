...
namespace ProjectApp.Services
{
    ...
    public class ApiService
    {
        ...
        public async Task SendHeartbeatAsync(int userId)
        {
            try
            {
                var payload = new { userId, deviceId = GetDeviceId() };
                var resp = await _http.PostAsJsonAsync($"{BaseUrl}/api/heartbeat", payload);
                System.Diagnostics.Debug.WriteLine($"[Heartbeat] userId={userId} → {(int)resp.StatusCode} ({BaseUrl})");
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[Heartbeat] FAILED userId={userId} url={BaseUrl} err={ex.Message}");
            }
        }

        /// <summary>Báo server offline ngay khi app tắt hoặc vào background.</summary>
        public async Task SendOfflineAsync(int userId)
        {
            try
            {
                using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(1));
                var payload = new { userId, deviceId = GetDeviceId() };
                await _http.PostAsJsonAsync($"{BaseUrl}/api/offline", payload, cts.Token);
                System.Diagnostics.Debug.WriteLine($"[Offline] userId={userId} sent");
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"[Offline] FAILED userId={userId} err={ex.Message}");
            }
        }

        /// <summary>GUID duy nhất per-install, dùng để theo dõi cả guest lẫn user đã login.</summary>
