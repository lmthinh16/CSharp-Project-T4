    ...
    public class AudioPlayerService : IAudioService, IDisposable
    {
        ...
        public async Task PlayAsync(AudioGuide guide)
        {
            ...
            try
            {
                ...
                // Tier 1: Tải MP3 về file tạm rồi phát — tránh stream không seekable trên Android
                if (!string.IsNullOrEmpty(guide.FilePath) && guide.FilePath.StartsWith("http"))
                {
                    string? tmpPath = null;
                    try
                    {
                        System.Diagnostics.Debug.WriteLine($"[Audio] Tier1: {guide.FilePath}");
                        var bytes = await _httpClient.GetByteArrayAsync(guide.FilePath, token);
                        token.ThrowIfCancellationRequested();

                        tmpPath = Path.Combine(FileSystem.CacheDirectory, $"audio_{version}.mp3");
                        await File.WriteAllBytesAsync(tmpPath, bytes, token);
                        token.ThrowIfCancellationRequested();

                        // CreatePlayer phải chạy trên main thread trên Android thật
                        _player = await MainThread.InvokeOnMainThreadAsync(() =>
                            Plugin.Maui.Audio.AudioManager.Current.CreatePlayer(tmpPath));

                        var tcs = new TaskCompletionSource();
                        _player.PlaybackEnded += (s, e) => tcs.TrySetResult();
                        token.Register(() => { _player?.Stop(); tcs.TrySetCanceled(); });
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine($"[Audio] Tier1 lỗi: {ex.Message}");
                        CleanUpPlayer();
                    }
                    finally
                    {
                ...
                // Tier 2: Text-To-Speech (không dùng locale cố định — tránh lỗi voice pack chưa cài)
                var ttsText = !string.IsNullOrEmpty(guide.TextContent) ? guide.TextContent
                            : !string.IsNullOrEmpty(guide.Title)       ? guide.Title
                            : null;

                if (!played && ttsText != null)
                {
                    token.ThrowIfCancellationRequested();
                    System.Diagnostics.Debug.WriteLine($"[Audio] Tier2 TTS: '{ttsText[..Math.Min(40,ttsText.Length)]}' lang={guide.LanguageCode}");
                    var langPrefix = guide.LanguageCode.Split('-')[0].ToLower();
                    var locales    = await TextToSpeech.Default.GetLocalesAsync();
                    var matched    = locales.FirstOrDefault(l =>
                                        l.Language.StartsWith(langPrefix, StringComparison.OrdinalIgnoreCase));
                    var opts = matched == null || string.IsNullOrEmpty(matched.Language)
                        ? new SpeechOptions { Volume = 1.0f, Pitch = 1.0f }
                        : new SpeechOptions { Volume = 1.0f, Pitch = 1.0f, Locale = matched };
                    await MainThread.InvokeOnMainThreadAsync(() =>
                        TextToSpeech.Default.SpeakAsync(ttsText, opts, token));
                    played = true;
                }
