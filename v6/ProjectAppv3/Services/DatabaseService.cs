...
namespace ProjectApp.Services
{
    ...
    public class DatabaseService
    {
        ...
        public async Task SyncRestaurantsAsync(List<Restaurant> apiData, string baseUrl = "")
        {
            ...
            {
                // Chuyển đường dẫn tương đối → URL đầy đủ để hiển thị ảnh
                if (!string.IsNullOrEmpty(baseUrl) && !string.IsNullOrEmpty(r.ImageUrl)
                    && !r.ImageUrl.StartsWith("http"))
                    r.ImageUrl = baseUrl.TrimEnd('/') + "/" + r.ImageUrl.TrimStart('/');
                ...
                if (r.Audios != null && r.Audios.Count > 0)
                {
                    foreach (var a in r.Audios)
                    {
                        ...
                        // Chuyển đường dẫn relative → URL đầy đủ để stream audio
                        if (!string.IsNullOrEmpty(baseUrl) && !string.IsNullOrEmpty(a.FilePath)
                            && a.FilePath != "OFFLINE_MODE" && !a.FilePath.StartsWith("http"))
                            a.FilePath = baseUrl.TrimEnd('/') + "/" + a.FilePath.TrimStart('/');
