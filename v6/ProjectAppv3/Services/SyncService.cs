...
namespace ProjectApp.Services
{
    ...
    public class SyncService
    {
        ...
        public async Task SyncAllAsync()
        {
            ...
            // ConstrainedInternet = có mạng nhưng bị giới hạn (hay gặp trên giả lập Android)
            var hasNet = Connectivity.NetworkAccess is NetworkAccess.Internet
        ...
        private async Task SyncRestaurantsAsync()
        {
            ...
            // Không có guard if (data.Count > 0) !
            // Nếu CMS xóa hết POI, API sẽ trả về [] → vẫn phải xóa local để đồng bộ.
            await _db.SyncRestaurantsAsync(data, _api.BaseUrl);
        }

        private async Task SyncToursAsync()
        {
            var data = await _api.GetToursAsync();
            // Tương tự: không guard, luôn sync để xóa tour đã bị xóa trên CMS.
            await _db.SyncToursAsync(data);
        }
