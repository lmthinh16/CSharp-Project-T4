...
namespace ProjectApp.ViewModels
{
    ...
    public partial class RestaurantDetailViewModel : ObservableObject
    {
        ...
        [ObservableProperty] string _favoriteIcon  = "🤍";   // emoji for Label binding
        [ObservableProperty] string _favoriteLabel = "❤️ Thêm"; // text for Button binding
        ...
        public RestaurantDetailViewModel()
        {
            ...
            Services.LocalizationManager.Instance.PropertyChanged += (_, _) =>
                MainThread.BeginInvokeOnMainThread(UpdateFavoriteUI);
        }
        ...
        async Task PlayAudio(AudioGuideItemVm item)
        {
            ...
            if (string.IsNullOrEmpty(guide.TextContent) && !guide.IsGeneratedByTTS && Restaurant != null)
            {
                var lang = guide.LanguageCode.Contains('-')
                    ? guide.LanguageCode.Split('-')[0].ToLower()
                    : guide.LanguageCode.ToLower();
                var script = Restaurant.GetTtsScript(lang);
                guide.TextContent = !string.IsNullOrEmpty(script) ? script : Restaurant.Name;
            }
        ...
        async Task ToggleFavorite()
        {
            ...
            // Notify FavoritePage via static event
            FavoritesViewModel.NotifyFavoritesChanged();
        }
        ...
        private void UpdateFavoriteUI()
        {
            ...
            FavoriteIcon  = isFav ? "❤️" : "🤍";           // top-nav Label
            FavoriteLabel = isFav ? L["detail_fav_saved"] : L["detail_fav_add"]; // bottom Button
        }
    }


    // ════════════════════════════════════════════════════════════
    //  FavoritesViewModel  –  FavoritePage
    // ════════════════════════════════════════════════════════════
    public partial class FavoritesViewModel : ObservableObject
    {
        // Static event to notify FavoritePage when any page changes favorites
        public static event EventHandler? FavoritesChanged;
        public static void NotifyFavoritesChanged() => FavoritesChanged?.Invoke(null, EventArgs.Empty);

        [ObservableProperty] ObservableCollection<Restaurant> _favorites = [];
        [ObservableProperty] bool _isRefreshing;
        [ObservableProperty] bool _isEmpty = true;
        [ObservableProperty] bool _isNotEmpty;
        [ObservableProperty] string _subtitleText = "";

        public FavoritesViewModel()
        {
            FavoritesChanged += async (_, _) =>
                await MainThread.InvokeOnMainThreadAsync(LoadAsync);
        }

        public async Task LoadAsync()
        {
            var list = await App.Database.GetFavoritesAsync();
            Favorites = new ObservableCollection<Restaurant>(list);
            IsEmpty = list.Count == 0;
            IsNotEmpty = !IsEmpty;
            SubtitleText = list.Count == 0
                ? "Chưa có quán yêu thích"
                : $"{list.Count} quán yêu thích";
        }
        ...
        [RelayCommand]
        async Task OpenDetail(Restaurant restaurant)
        {
            await Shell.Current.GoToAsync("RestaurantDetail",
        ...
        [RelayCommand]
        async Task RemoveFavorite(Restaurant restaurant)
        {
            await App.Database.ToggleFavoriteAsync(restaurant.Id);
